...
import com.vonagevoice.utils.tryNotBlocking
...
class VonageVoiceModule(reactContext: ReactApplicationContext) :
    ...
    fun hangup(callId: String, promise: Promise) {
        Log.d("VonageVoiceModule", "hangup callId: $callId")
        val normalizedCallId = callId.lowercase()
        Log.d("VonageVoiceModule", "hangup normalizedCallId: $normalizedCallId")
    ...
    fun setAudioDevice(deviceId: String, promise: Promise) {
        ...
        promise.tryNotBlocking {
            deviceManager.setAudioDevice(
                deviceId.toIntOrNull()
                    ?: throw IllegalStateException("Device id not supported $deviceId")
            )
        }
    }
